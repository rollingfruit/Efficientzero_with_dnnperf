...
class ANEE(nn.Module):
    ...
    def forward(self, node_features, edge_features, edges):
        ...
        h_u_bar = h_u_bar.unsqueeze(1)
        h_u_bar, _ = self.multihead_attention(h_u_bar, h_u_bar, h_u_bar)
        h_u_bar = h_u_bar.squeeze(1)
        
        h_u = torch.zeros_like(h_u_bar)
        e_i = torch.zeros(edge_features.size(0), self.W_e.out_features, device=node_features.device)
        # 将edges从列表转换为张量
        edges_tensor = torch.tensor(edges, dtype=torch.long, device=node_features.device)
        src_indices, tgt_indices = edges_tensor.T
        # 向量化边缘影响计算
        edge_features = edge_features.unsqueeze(1)  # 为了与W_e相乘
        # src_indices, tgt_indices = edges.T
        src_features = h_u_bar[src_indices]
        tgt_features = h_u_bar[tgt_indices]
        src_tgt_features = torch.cat((src_features, tgt_features), dim=1)
        e_i = torch.sigmoid(self.W_e(edge_features) * torch.matmul(self.attention, src_tgt_features.T).T)
        

        # for i, (src, tgt) in enumerate(edges_tensor):
        #     # 确保 e_i[i] 是标量，正确应用于 h_u_bar[tgt]
        #     # 由于 e_i[i] 是标量，它将自动广播以匹配 h_u_bar[tgt] 的形状
        #     print("e_i[i].squeeze().shape: ",e_i[i].squeeze().shape)

        #     h_u[src] += e_i[i].squeeze() * h_u_bar[tgt]
        # 假设修正的权重计算
        # 这里我们需要一个新的方法来计算 e_i，使其为每条边生成一个标量权重
        # 以下代码仅为示意，实际实现应根据您的具体需求调整

        for i, (src, tgt) in enumerate(edges):
            # 假设的权重计算，需要替换为实际的计算方法
            e_i_scalar = torch.sigmoid(self.W_e(edge_features[i].unsqueeze(0))).sum()
            e_i[i] = e_i_scalar

        # 使用修正后的 e_i 更新节点特征
        for i, (src, tgt) in enumerate(edges):
            # 现在 e_i[i] 应该是一个标量
            h_u[src] += e_i[i] * h_u_bar[tgt]
