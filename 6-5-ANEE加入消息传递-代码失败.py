...
class ANEE(MessagePassing):
    ...
    def forward(self, node_features, edge_index, edge_features):
        ...
        # 处理 edge_index，确保其为整数类型的张量（torch.long）
        if isinstance(edge_index, list) or (isinstance(edge_index, torch.Tensor) and edge_index.dtype != torch.long):
            edge_index = torch.tensor(edge_index, dtype=torch.long)
        if edge_index.dim() == 2 and edge_index.size(0) != 2:
            # 如果edge_index的第一维不是2，则需要转置
            edge_index = edge_index.t().contiguous()
        elif edge_index.dim() == 1:
            # 如果edge_index是一维的，则需要重新检查其结构，这可能是一个异常情况
            raise ValueError("edge_index is one-dimensional, which is not expected. Please check its structure.")
        # 确保 edge_index 的形状是 [2, E]
        if edge_index.size(0) != 2:
            edge_index = edge_index.t()
        edge_features_tensor = torch.tensor(edge_features, dtype=torch.float)  # 注意这里使用了 torch.float

        # 调用propagate方法开始消息传递过程
        h_u = self.propagate(edge_index, size=(h_u_bar.size(0), h_u_bar.size(0)), node_features=h_u_bar, edge_features=edge_features_tensor)
        
        return F.leaky_relu(h_u), None

    def message(self, edge_index_i, node_features_i, node_features_j, edge_features, size_i):
        # edge_index_i: 目标节点索引
        # node_features_i: 源节点的特征
        # node_features_j: 目标节点的特征
        # edge_features: 边的特征
        # size_i: 输出节点的数量

        src_tgt_features = torch.cat((node_features_i, node_features_j), dim=-1)
        e_i = torch.sigmoid(self.W_e(edge_features.unsqueeze(-1)) * torch.matmul(self.attention, src_tgt_features.unsqueeze(-1)))

        weighted_edges = softmax(e_i, edge_index_i, num_nodes=size_i)
        return weighted_edges * node_features_j

    def update(self, aggr_out):
        # aggr_out: 聚合后的节点特征
        return torch.matmul(aggr_out, self.W_m)
...
# edge_features_tensor_train_third_col
