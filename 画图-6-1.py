...
def generate_features_from_folders(node_folder_path, edge_folder_path):
    ...
    for node_file_name in os.listdir(node_folder_path):
        ...
        # with open(node_csv_path, mode='r', encoding='ascii') as node_file:
...
new_prediction_test = adjust_prediction_to_match_numpy(prediction_test, runtimes_tensor_test, 30)
...
# plt.plot([0, 4], [0, 4], 'r--')  # 添加y=x参考线
plt.plot([0, max(actual)], [0, max(actual)], 'r--')  # 理想预测线
plt.savefig('./figs/5-1Actual vs Predicted Runtime.png')
plt.show()
