...
# 经典
# optimizer = torch.optim.SGD(list(anee_model.parameters()) + list(prediction_layer.parameters()), lr=0.01, momentum=0.9)  # SGD with momentum
# optimizer = torch.optim.Adagrad(list(anee_model.parameters()) + list(prediction_layer.parameters()), lr=0.01)  # Adagrad
# optimizer = torch.optim.RMSprop(list(anee_model.parameters()) + list(prediction_layer.parameters()), lr=0.01, alpha=0.99)  # RMSprop

# 进阶
# optimizer = torch.optim.Adadelta(list(anee_model.parameters()) + list(prediction_layer.parameters()), lr=1.0)  # Adadelta
# optimizer = torch.optim.Adam(list(anee_model.parameters()) + list(prediction_layer.parameters()), lr=0.001)  # Adam

# 较新
# optimizer = torch.optim.Adamax(list(anee_model.parameters()) + list(prediction_layer.parameters()), lr=0.002)  # Adamax
...
# optimizer = torch.optim.ASGD(list(anee_model.parameters()) + list(prediction_layer.parameters()), lr=0.01)  # ASGD

# 更适合
"""
    本场景下，需要构建并训练一个深度学习模型（ANEE），是高度非凸的，且包含大量参数，适合以下几种：
1. Adam，是深度学习领域非常流行的优化器，它结合了动量（Momentum）和自适应学习率（Adaptive Learning Rate）的优点，适合于处理大规模和非凸优化问题。
2. AdamW，是Adam的一个变体，它在权重衰减方面进行了改进。在正则化和避免过拟合方面，比标准Adam在深度学习模型中有更好的泛化能力
3. RAdam (Rectified Adam)。RAdam是Adam的另一个变体，它通过引入一个可调节的长期方差项来解决Adam在训练早期的不稳定性问题。使得RAdam在一些情况下比标准Adam更稳定
4. NAdam (Nesterov-accelerated Adaptive Moment Estimation)。NAdam结合了Adam和Nesterov动量的优点。Nesterov动量可以在某些情况下加速训练，并可能提供更平滑的优化轨迹。
"""
# optimizer = torch.optim.Adam(list(anee_model.parameters()) + list(prediction_layer.parameters()), lr=0.001)  # Adam
